    ...
    return (
        <Drawer anchor="right" open={open} onClose={onClose}
            ...
            <p className="text-xs text-(--text-secondary) font-medium mb-6">
                {editing ? "Modify the intake capacity for this session." : "Define a new academic year for the selected department."}
            </p>

            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                <TextField fullWidth label="Start Year (e.g. 2026)" type="number" required disabled={!!editing}
                    value={startYear} onChange={(e) => setStartYear(Number(e.target.value))}
                    sx={textFieldSx} />

                <TextField fullWidth label="Intake Capacity (Optional)" type="number"
                    value={intakeCapacity} onChange={(e) => setIntakeCapacity(Number(e.target.value))}
                    sx={textFieldSx} />
                ...
                <div className="flex gap-3 pt-4 border-t border-(--ui-divider) mt-2">
                    ...
                    <Button fullWidth type="submit" variant="contained" disabled={isLoading || (!editing && !startYear)}
                        sx={{ ...buttonSx, bgcolor: "var(--brand-primary)", boxShadow: "none", "&:hover": { bgcolor: "var(--bg-sidebar)", boxShadow: "none" } }}>
                        {isLoading ? "Saving…" : editing ? "Update Session" : "Create Session"}
                    </Button>
                </div>
            </form>
        </Drawer>
        ...
        <Dialog open={open} onClose={onClose} PaperProps={{ sx: { borderRadius: "12px", border: "1px solid var(--ui-border)", bgcolor: "var(--bg-surface)", minWidth: 400, boxShadow: "0 10px 25px -5px rgb(0 0 0 / 0.1)" } }}>
            ...
            <DialogContent sx={{ px: 3 }}>
                <p className="text-sm text-(--text-secondary) font-medium mb-4">
                    Advance the current semester for the <strong>{session?.startYear} - {session?.endYear}</strong> session. This will affect all enrolled students.
                </p>
                <TextField fullWidth label="Reason for Manual Promotion" required multiline rows={3}
                    value={reason} onChange={(e) => setReason(e.target.value)}
                    sx={textFieldSx} />
            </DialogContent>
            <DialogActions sx={{ p: 3, pt: 2 }}>
                <Button onClick={onClose} sx={{ color: "var(--text-secondary)", fontWeight: 600, textTransform: "none" }}>Cancel</Button>
                <Button onClick={handlePromote} disabled={isLoading || !reason.trim()}
...
const AcademicSessions = () => {
    ...
    const { data: sessionData, isLoading: sessionsLoading } = useGetSessionsByDeptQuery(
    ...
    const handleLockToggle = async (session: AcademicSession) => {
        ...
        try {
            if (session.status === "locked") {
                await unlockSession({ deptId: selectedDept, id: session._id }).unwrap();
            } else {
                await lockSession({ deptId: selectedDept, id: session._id }).unwrap();
            }
        } catch (_) { }
    };
    ...
    const handleCloseEnrollment = async (session: AcademicSession) => {
        if (!selectedDept) return;
        try {
            await closeEnrollment({ deptId: selectedDept, id: session._id }).unwrap();
        } catch (_) { }
    };
    ...
    return (
        <div className="w-full bg-[var(--bg-base)] min-h-screen font-sans pb-10">
                ...
                <div className="mb-8 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-(--text-secondary) text-sm font-medium mt-1">
                            Configure institutional terms, capacities, and enrollment states.
                        </p>
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        <Button variant="contained" startIcon={<Plus size={16} />}
                            ...
                            onClick={() => { setEditingSession(null); setDrawerOpen(true); }}
                                ...
                                {!selectedDept ? (
                                    <tr>
                                        <td colSpan={6} className="px-6 py-16 text-center">
                                            <BookOpen size={40} className="mx-auto text-(--text-secondary) opacity-30 mb-3" />
                                            <p className="font-bold text-(--text-primary) text-base">Select a Department</p>
                                            <p className="text-xs text-(--text-secondary) mt-1">Choose a department from the dropdown to view its sessions.</p>
                                        </td>
                                    </tr>
                                ) : sessionsLoading ? (
                                        ...
                                        <tr key={session._id} className="hover:bg-[var(--bg-base)] transition-colors group">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <p className="font-bold text-sm text-(--text-primary)">{session.startYear} - {session.endYear}</p>
                                                        <p className="text-[11px] text-(--text-secondary) font-bold uppercase tracking-wider mt-0.5">Session Term</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col">
                                                    <span className="text-sm font-bold text-(--text-primary)">{session.totalEnrolledStudents} <span className="text-(--text-secondary) font-medium">/ {session.intakeCapacity || "∞"}</span></span>
                                                    <span className="text-[10px] text-(--text-secondary) uppercase tracking-wider font-bold mt-0.5">Students</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <Chip
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <IconButton size="small" title="Edit Capacity"
                                                        onClick={() => { setEditingSession(session); setDrawerOpen(true); }}
                                                        sx={{ color: "var(--text-secondary)", "&:hover": { color: "var(--brand-primary)", bgcolor: "var(--brand-active)" } }}>
                                                        <Pencil size={16} />
                                                    </IconButton>

                                                    {session.enrollmentOpen && (
                                                        <IconButton size="small" title="Close Enrollment"
                                                            onClick={() => handleCloseEnrollment(session)}
                                                    ...
                                                    {(session.status === "active" || session.status === "locked") && (
                                                        <IconButton size="small" title={session.status === "locked" ? "Unlock Session" : "Lock Session"}
                                                            onClick={() => handleLockToggle(session)}
                                                            sx={{ color: "var(--text-secondary)", "&:hover": { color: "var(--brand-primary)", bgcolor: "var(--brand-active)" } }}>
                                                            {session.status === "locked" ? <Unlock size={16} /> : <Lock size={16} />}
                                                        </IconButton>
                                                    )}

                                                    {session.status === "active" && (
                                                        <IconButton size="small" title="Promote Semester"
                                                            onClick={() => setPromoteSession(session)}
                                                            sx={{ color: "var(--text-secondary)", "&:hover": { color: "var(--brand-primary)", bgcolor: "var(--brand-active)" } }}>
            ...
            <SessionDrawer
                ...
                onClose={() => { setDrawerOpen(false); setEditingSession(null); }}
            />
            <PromoteDialog
                open={!!promoteSession}
