...
const StatCard = ({
    ...
    <div className="p-5 bg-(--bg-surface) border border-(--ui-border) rounded-xl hover:border-(--brand-primary) transition-colors shadow-sm">
        <div className="flex justify-between items-start mb-3">
            <div
                className={`p-2.5 rounded-lg bg-(--bg-base) w-fit ${colorClass}`}
            >
        ...
        {loading ? (
            <Skeleton
                width={48}
                height={32}
                sx={{ bgcolor: "var(--ui-divider)", mb: 0.5 }}
            />
        ) : (
...
const QuickAction = ({
    ...
    >
        ...
        <ChevronRight
            size={16}
            className="text-(--text-secondary) group-hover:text-(--brand-primary) group-hover:translate-x-1 transition-all"
        />
    </a>
);
                        ...
                        <section className="bg-(--bg-surface) rounded-xl border border-(--ui-border) p-6 shadow-sm">
                            ...
                            <div className="space-y-1">
                                <QuickAction
                                    ...
                                    desc="Create, edit or assign HODs"
                                    icon={BookOpen}
                                    href="/principal/departments"
                                />
                                <QuickAction
                                    label="HOD Management"
                                    desc="View and manage all HODs"
                                    icon={Users}
                                    href="/principal/hods"
                                />
                                <QuickAction
                                    label="Academic Sessions"
                                    desc="View and lock sessions"
                                    icon={CalendarDays}
                                    href="/principal/sessions"
                                />
                                <QuickAction
                                    label="Institute Profile"
                                    desc="Update institute information"
                                    icon={Building2}
                                    href="/principal/institute"
                                />
                                <QuickAction
                            ...
                            <div className="grow overflow-y-auto custom-scrollbar pr-1">
                                {deptLoading ? (
                                    ...
                                    <div className="h-full flex flex-col items-center justify-center text-(--text-secondary) opacity-60 py-8">
                                        ...
                                        <a
                                            href="/principal/departments"
                                            className="text-xs text-(--brand-primary) font-bold hover:underline mt-1"
                                        >
                                    ...
                                    <div className="space-y-2.5">
                                        {departments.slice(0, 5).map((dept) => (
                                            <div
                                                key={dept._id}
                                                className="flex items-center justify-between p-3.5 rounded-xl border border-(--ui-border) bg-(--bg-base) hover:border-(--brand-primary) transition-colors"
                                            >
                                                <div>
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <span
                                                        className={`w-2 h-2 rounded-full ${dept.isActive ? "bg-emerald-500 shadow-[0_0_6px_rgba(16,185,129,0.4)]" : "bg-rose-500 shadow-[0_0_6px_rgba(244,63,94,0.4)]"}`}
                                                    />
                                                </div>
                                            </div>
                                        ))}
