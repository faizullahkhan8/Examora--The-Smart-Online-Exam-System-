...
const defaultQ = (type: QType = "short"): Question => ({
    ...
    text: "",
    marks:
        type === "mcq" ? 2 : type === "short" ? 5 : type === "long" ? 10 : 20,
    difficulty: "medium",
    bloom: undefined,
    options:
        type === "mcq"
...
const qTypeLabels: Record<QType, string> = {
    mcq: "MCQ",
    short: "Short",
    long: "Long",
    case: "Case Study",
        ...
        <div className="flex items-center gap-4">
            <svg width={72} height={72} viewBox="0 0 72 72">
                ...
                <circle
                    ...
                    stroke="#f59e0b"
                    strokeWidth={10}
                    strokeDasharray={`${mD} ${circ - mD}`}
                    strokeDashoffset={circ / 4 - eD}
                />
                {/* Hard */}
                <circle
                    cx={36}
        ...
        <div className="flex items-center gap-3">
            <TextField
                ...
                onChange={(e) =>
                    onChange({
                        ...q,
                        wordLimit: Number(e.target.value) || undefined,
                    })
                }
            />
        </div>
...
const CaseStudyCard = ({
    ...
    const setSub = (i: number, field: keyof SubQuestion, val: any) =>
        onChange({
            ...q,
            subQuestions: q.subQuestions!.map((s, j) =>
                j === i ? { ...s, [field]: val } : s,
            ),
        });
    const addSub = () =>
        onChange({
            ...q,
            subQuestions: [...q.subQuestions!, { text: "", marks: 5 }],
        });
    const removeSub = (i: number) =>
        onChange({
            ...q,
            subQuestions: q.subQuestions!.filter((_, j) => j !== i),
        });
            ...
            <div className="border-l-2 border-orange-200 pl-4 space-y-3">
                ...
                {q.subQuestions!.map((s, i) => (
                    <div key={i} className="flex items-start gap-2">
                        <span className="text-xs font-black text-slate-400 mt-2.5 w-5 shrink-0">
                        ...
                        <TextField
                            ...
                            sx={{ width: 80 }}
                            onChange={(e) =>
                                setSub(i, "marks", Number(e.target.value))
                            }
                        />
                        {q.subQuestions!.length > 1 && (
                            <button
                                onClick={() => removeSub(i)}
                                className="text-rose-400 hover:text-rose-600 mt-2"
                            >
                                <Trash2 size={14} />
                            </button>
                        )}
...
const QuestionCard = ({
    ...
    const actualMarks =
        q.type === "case"
            ? q.subQuestions!.reduce((a, s) => a + s.marks, 0)
            : q.marks;
        ...
        <div className="border border-slate-200 rounded-2xl bg-white overflow-hidden shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
                ...
                <div className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <span
                        className={`text-[10px] font-black px-2 py-0.5 rounded-full ${qTypeColors[q.type]}`}
                    >
                        {qTypeLabels[q.type]}
                    </span>
                    {q.text && (
                ...
                <div className="p-4 space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            ...
                            onChange={(e) =>
                                onChange({
                                    ...q,
                                    bloom:
                                        (e.target.value as Bloom) || undefined,
                                })
                            }
                        ...
                        >
                            ...
                            {(
                                Object.entries(bloomLabels) as [Bloom, string][]
                            ).map(([k, v]) => (
                    ...
                    {(q.type === "short" || q.type === "long") && (
                        <SubjectiveCard q={q} onChange={onChange} />
                    )}
                    {q.type === "case" && (
                        <CaseStudyCard q={q} onChange={onChange} />
                    )}
                </div>
            )}
        </div>
...
const SectionPanel = ({
    ...
    const marks = section.questions.reduce((a, q) => {
        if (q.type === "case")
            return a + q.subQuestions!.reduce((b, s) => b + s.marks, 0);
        return a + q.marks;
    }, 0);
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <div
                        className={`w-2 h-2 rounded-full ${isActive ? "bg-indigo-500" : "bg-slate-300"}`}
                    />
...
const PreviewMode = ({
    ...
    const totalAssigned = sections.reduce(
        (a, sec) =>
            a +
            sec.questions.reduce(
                (b, q) =>
                    b +
                    (q.type === "case"
                        ? q.subQuestions!.reduce((c, s) => c + s.marks, 0)
                        : q.marks),
                0,
            ),
        0,
            ...
            <div className="max-w-[800px] mx-auto p-12 print:p-8">
                ...
                <div className="text-center border-b-2 border-slate-900 pb-6 mb-8">
                    ...
                    <div className="flex justify-center gap-10 mt-4 text-sm font-semibold text-slate-600">
                        ...
                        <span>
                            Total Marks: <strong>{totalAssigned}</strong>
                        </span>
                        <span>
                            Duration: <strong>{meta.duration} min</strong>
                        </span>
                    </div>
                                ...
                                return (
                                    <div key={q.id}>
                                        <div className="flex items-start justify-between">
                                            ...
                                            <span className="text-sm font-black text-slate-700 ml-4 shrink-0">
                                                [{marks} M]
                                            </span>
                                        </div>
                                        {q.type === "mcq" && (
                                        ...
                                        {(q.type === "short" ||
                                            q.type === "long") && (
                                                <div
                                                    className={`mt-3 ml-5 border-b border-dashed border-slate-200 ${q.type === "long" ? "h-40" : "h-20"}`}
                                                />
                                            )}
                                    </div>
                                );
...
const ExamPaperBuilder = () => {
        ...
        const qs = (paper.questions ?? []).map((q: any) => ({
            ...
            options:
                ...
                (q.type === "mcq"
                    ...
                    : undefined),
            subQuestions:
                q.type === "case" ? [{ text: "", marks: 5 }] : undefined,
        }));
        if (qs.length > 0) {
            setSections([
                {
    ...
    const deleteSection = (id: string) =>
        setSections((ss) => {
            const f = ss.filter((s) => s.id !== id);
            setActiveSecId(f[0]?.id ?? "");
            return f;
        });
    ...
    const addQuestion = (secId: string, type: QType) => {
        setSections((ss) =>
            ss.map((s) =>
                s.id === secId
                    ? { ...s, questions: [...s.questions, defaultQ(type)] }
                    : s,
            ),
        );
    };
        ...
        setSections((ss) =>
            ss.map((s) =>
                s.id === secId
                    ? {
                        ...s,
                        questions: s.questions.map((qi, i) =>
                            i === qIdx ? q : qi,
                        ),
                    }
                    : s,
    ...
    useEffect(() => {
        const t = setInterval(() => {
            if (meta.subjectId && meta.title) handleSave(true);
        }, 10000);
    ...
    const handleSubmit = async () => {
        if (!isValid) {
            toast.error("Fix all validation issues before submitting");
            return;
        }
        try {
            let id = paperId;
                ...
                <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                        {[
                            ...
                            {
                                label: "Marks OK",
                                done: !marksMismatch && totalAssigned > 0,
                            },
                            {
                                label: "No empty Qs",
                                done: !hasEmptyQ && totalQuestions > 0,
                            },
                        ].map((step, i) => (
                            <div key={i} className="flex items-center gap-1">
                                {i > 0 && (
                                    <ChevronDown
                                        size={10}
                                        className="rotate-[-90deg] text-slate-300"
                                    />
                                )}
                                <span
                                    className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full ${step.done ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-400"}`}
                                >
                ...
                <aside className="w-[240px] shrink-0 bg-white border-r border-slate-200 flex flex-col overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2">
                        ...
                        {sections.map((sec, si) => (
                            <SectionPanel
                                ...
                                onChange={(s) => updateSection(s.id, s)}
                                onDelete={() => deleteSection(sec.id)}
                                onMoveUp={() => moveSec(si, -1)}
                                onMoveDown={() => moveSec(si, 1)}
                                isActive={sec.id === activeSecId}
                    ...
                    <div className="px-4 py-3 border-t border-slate-100 bg-slate-50/50">
                        <div className="flex justify-between text-xs font-semibold text-slate-500 mb-1">
                            ...
                            <span
                                className={`font-black ${marksMismatch && totalAssigned > 0 ? "text-rose-600" : "text-emerald-600"}`}
                            >
                        ...
                        <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden">
                            <div
                                className={`h-full rounded-full transition-all ${marksMismatch && totalAssigned > 0 ? "bg-rose-500" : "bg-emerald-500"}`}
                                style={{
                                    width: `${Math.min(100, (totalAssigned / meta.totalMarks) * 100)}%`,
                                }}
                            />
                        </div>
                    </div>
                            ...
                            {activeSec.questions.length === 0 ? (
                                <div className="p-16 text-center border-2 border-dashed border-slate-200 rounded-3xl text-slate-400">
                                    ...
                                    <p className="text-sm mt-1">
                                        Use the buttons below to add your first
                                        question.
                                    </p>
                                </div>
                            ) : (
                                <div className="space-y-4">
                                    {activeSec.questions.map((q, qi) => (
                                        <QuestionCard
                                            ...
                                            onDelete={() =>
                                                deleteQuestion(activeSec.id, qi)
                                            }
                                            onMoveUp={() =>
                                                moveQ(activeSec.id, qi, -1)
                                            }
                                            onMoveDown={() =>
                                                moveQ(activeSec.id, qi, 1)
                                            }
                ...
                <aside className="w-[260px] shrink-0 bg-white border-l border-slate-200 flex flex-col overflow-y-auto">
                    ...
                    <div className="px-4 py-4 border-b border-slate-100 space-y-2.5">
                        ...
                        <div className="bg-slate-50 rounded-xl p-3">
                            ...
                            <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                                <div
                                    ...
                                    style={{
                                        width: `${Math.min(100, (totalAssigned / (meta.totalMarks || 1)) * 100)}%`,
                                    }}
                                />
                            </div>
                        </div>
                        ...
                        {[
                            ...
                            {
                                label: "Marks match exactly",
                                ok: !marksMismatch && totalAssigned > 0,
                            },
                            {
                                label: "No empty questions",
                                ok: !hasEmptyQ && totalQuestions > 0,
                            },
                        ].map(({ label, ok }) => (
                            <div
                                key={label}
                            ...
                            >
                                ...
                                <span
                                    className={`text-xs font-semibold ${ok ? "text-slate-600" : "text-rose-500"}`}
                                >
                                    {label}
                                </span>
                            </div>
                    ...
                    <div className="px-4 py-4">
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-1">
                            {(
                                ...
                                <div
                                    key={k}
                                    className={`${cls} rounded-lg px-2 py-1.5 text-center`}
                                >
