...
const MaterialsPage = () => {
    ...
    const [form, setForm] = useState({
        title: "",
        type: "link" as "pdf" | "link" | "note",
        content: "",
    });

    const { data, isLoading } = useGetMaterialsBySubjectQuery(selectedSubject, {
    ...
    const handleCreate = async () => {
        if (!selectedSubject || !form.title || !form.content)
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-sm font-medium text-slate-500">
                        Share notes, links, and PDFs with students
                    </p>
                </div>
                <Button
                    variant="contained"
            ...
            {!selectedSubject ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-pulse">
                    {[1, 2, 3].map((i) => (
                        <div
                            key={i}
                            className="h-36 bg-slate-100 rounded-2xl"
                        />
                    ))}
                </div>
            ) : materials.length === 0 ? (
                <Paper
                    elevation={0}
                    className="p-12 text-center border border-slate-200 rounded-2xl text-slate-400"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {materials.map((m: any) => (
                        <Paper
                            key={m._id}
                            elevation={0}
                            className="p-5 rounded-2xl border border-slate-200 bg-white hover:shadow-md transition-all group"
                        >
            ...
            >
                ...
                <DialogContent className="space-y-4 pt-2">
                    ...
                    <TextField
                        ...
                        multiline={form.type === "note"}
                        rows={form.type === "note" ? 4 : 1}
                        value={form.content}
                        onChange={(e) =>
