...
const SubjectCard = ({ subject }: { subject: any }) => {
    ...
    return (
        <Paper
            elevation={0}
            className="p-6 rounded-2xl border border-slate-200 bg-white hover:shadow-md transition-all group"
        >
            <div className="flex items-start justify-between mb-4">
                ...
                <Chip
                    label={subject.session?.status ?? "active"}
                    size="small"
            ...
            <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-100">
                <Button
                    component={Link}
                    to={`/teacher/exam-papers?subject=${subject._id}`}
                    size="small"
                ...
                <Button
                    component={Link}
                    to={`/teacher/attendance?subject=${subject._id}`}
                    size="small"
                    startIcon={<CalendarCheck size={13} />}
                    variant="outlined"
                    className="!text-[11px] font-bold! normal-case! rounded-xl! border-slate-200! !text-slate-600"
                >
                    Attendance
                ...
                <Button
                    component={Link}
                    to={`/teacher/materials?subject=${subject._id}`}
                    size="small"
                    startIcon={<BookMarked size={13} />}
                    variant="outlined"
                    className="!text-[11px] font-bold! normal-case! rounded-xl! border-slate-200! !text-slate-600"
                >
                    Materials
        ...
        <div className="p-8 pb-32 max-w-[1200px] mx-auto w-full font-sans">
            <div className="mb-8">
                ...
                <p className="text-sm font-medium text-slate-500">
                    All subjects currently assigned to you
                </p>
            </div>
            ...
            {isLoading ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-pulse">
                    {[1, 2, 3].map((i) => (
                        <div
                            key={i}
                            className="h-56 bg-slate-100 rounded-2xl"
                        />
                    ))}
                </div>
            ) : subjects.length === 0 ? (
