...
const AttendancePage = () => {
    ...
    const { data: studentsData, isLoading: studentsLoading } =
        useGetSubjectStudentsQuery(selectedSubject, { skip: !selectedSubject });
    const { data: historyData } = useGetAttendanceBySubjectQuery(
        selectedSubject,
    ...
    useEffect(() => {
        ...
        const existing = historyData?.data?.find(
            (a) => new Date(a.date).toISOString().slice(0, 10) === date,
        );
        if (existing) {
            const map: Record<string, boolean> = {};
            existing.records.forEach((r: any) => {
                map[typeof r.student === "object" ? r.student._id : r.student] =
                    r.present;
            });
            setAttendance(map);
        } else {
                            ...
                            <tbody>
                                {students.map((s: any) => {
                                    ...
                                    return (
                                        <tr
                                            key={s._id}
                                            className={`border-b border-slate-100 transition-colors ${present ? "bg-white" : "bg-rose-50/30"}`}
                                        >
